...
export class ConfigService implements OnModuleInit {
  ...
  /**
   * Recursively scans for .properties files in the project directory.
   * @param directory - The directory to start scanning from.
   */
  private async scanForPropertiesFiles(directory: string) {
    const files = await fs.promises.readdir(directory, { withFileTypes: true });
    for (const file of files) {
      const fullPath = path.join(directory, file.name);
      if (file.isDirectory()) {
        await this.scanForPropertiesFiles(fullPath);
        ...
        if (fs.existsSync(localPropertiesPath)) {
          ...
          this.logger.verbose(`Create file ${localPropertiesPath}`);
          await fs.promises.writeFile(localPropertiesPath, "", FILES_ENCODING);
        }
        this.propertiesFiles[fullPath] = await this.processAndValidatePropertiesContent(fullPath, fileContent);
      }
    }
  }

  /**
   * Processes and validates the properties content.
   * @param filePath - The path of the properties file.
   * @param content - The content of the properties file.
   * @returns An object containing the processed key-value pairs.
   */
  private async processAndValidatePropertiesContent(filePath: string, content: string) {
